...
public class TaskMang extends BaseClass {
   ...
   public void editProject() throws InterruptedException {
	   com.olobby.ObjectRepo.HomePage hp = new com.olobby.ObjectRepo.HomePage(driver);
	   Thread.sleep(2000);
...
//	   ap.getEnterProjectName().sendKeys(eLib.readDataFromExcel("EditProject", 1, 6));
	   ea.getLocation().sendKeys(eLib.readDataFromExcel("EditProject", 1, 7));
	   ea.clickEnter(driver);
	   ea.getProjectName().sendKeys(eLib.readDataFromExcel("EditProject", 1, 8));
	   ea.clickEnter(driver);
	 
	   ea.getStartDate().clear();
	   ea.getStartDate().sendKeys(eLib.readDataFromExcel("EditProject", 1, 9));

	   ea.getDueDate().clear();
	   
	   ea.getDueDate().sendKeys(eLib.readDataFromExcel("EditProject", 1, 10));
